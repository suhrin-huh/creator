...
export async function saveSponsorship(
  ...
  const deadlineDays = formData.get('deadlineDays') ? Number(formData.get('uploadPeriodDays')) : null;
  const retentionMonths = formData.get('retentionMonths') ? Number(formData.get('contentRetentionMonths')) : null;
